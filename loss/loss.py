...
def diversity_loss(qf,gf):
    b,k,d = qf.shape

    epsilon = 0.000001
    dist_mat = qf.bmm(gf.permute(0,2,1))
    qf_norm = torch.norm(qf, p=2, dim=2, keepdim=True)
    gf_norm = torch.norm(gf, p=2, dim=2, keepdim=True)
    qg_normdot = qf_norm.bmm(gf_norm.permute(0,2,1))
    qg_normdot = torch.clamp(qg_normdot,-1 + epsilon, 1 - epsilon)
    dist_mat = dist_mat.mul(1 / qg_normdot)
    mask = torch.eye(k).ne(torch.ones(k,k)).unsqueeze(0).expand(b,k,k)
    dist_mat = dist_mat.mul(mask)
    dist_mat = torch.sum(dist_mat,dim=2).mul(1 / (k*(k-1)))
    dist_mat = torch.sum(dist_mat)
...
class Diversity_loss(nn.Module):
    ...
    def forward(self, qf,gf):
        ...
        gf_norm = torch.norm(gf, p=2, dim=2, keepdim=True)
        qg_normdot = qf_norm.bmm(gf_norm.permute(0, 2, 1))
        qg_normdot = torch.clamp(qg_normdot, -1 + self.epsilon, 1 - self.epsilon)
        dist_mat = dist_mat.mul(1 / qg_normdot)
        mask = torch.eye(k).ne(torch.ones(k, k)).unsqueeze(0).expand(b, k, k)
        dist_mat = dist_mat.mul(mask)
        dist_mat = torch.sum(dist_mat, dim=2).mul(1 / (k * (k - 1)))
