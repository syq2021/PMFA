...
# from model.model_agw_1205pose_MAM import embed_net
...
if args.method =='base':
    net = embed_net(n_class, no_local= 'off', gm_pool = 'off', pcb= 'off', aligned=True, arch=args.arch)
else:
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        test_trial = trial +1
        # model_path = checkpoint_path +  args.resume
        model_path = checkpoint_path + 'regdb_base_p4_n8_lr_0.1_seed_0_trial_{}_best.t'.format(test_trial)
        if os.path.isfile(model_path):
